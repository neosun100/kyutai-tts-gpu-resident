...
# 离线模式：直接使用本地模型路径
MODEL_PATH = '/app/models/models--kyutai--tts-1.6b-en_fr/snapshots'
VOICE_PATH = '/app/models/models--kyutai--tts-voices/snapshots'

def get_snapshot_path(base_path):
    """获取 snapshot 目录"""
    snapshots = os.path.join(base_path)
    if os.path.exists(snapshots):
        dirs = [d for d in os.listdir(snapshots) if os.path.isdir(os.path.join(snapshots, d))]
        if dirs:
            return os.path.join(snapshots, dirs[0])
    return None

def load_model():
    model_dir = get_snapshot_path(MODEL_PATH)
    if not model_dir:
        raise RuntimeError(f"Model not found in {MODEL_PATH}")
    
    checkpoint_info = CheckpointInfo.from_pretrained(model_dir)
...
def tts():
    ...
    try:
        ...
        # 使用默认 voice
        voice_dir = get_snapshot_path(VOICE_PATH)
        voice_file = os.path.join(voice_dir, 'expresso/ex03-ex01_happy_001_channel1_334s.wav.safetensors')
        
        condition_attributes = tts_model.make_condition_attributes([voice_file], cfg_coef=cfg_coef)
...
<script>
const i18n={
en:{title:"Kyutai TTS",gpu_status:"GPU Status: ",offload_gpu:"Release GPU",text_label:"Text to Synthesize",generate:"Generate"},
"zh-CN":{title:"Kyutai 语音合成",gpu_status:"GPU 状态: ",offload_gpu:"释放显存",text_label:"输入文本",generate:"生成语音"}
};
